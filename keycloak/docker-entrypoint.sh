...
export CATALOG_OIDC_CLIENT_SECRET="${CATALOG_OIDC_CLIENT_SECRET:-biblio-catalog-secret-key-2026}"
...
if [ -f "$TEMPLATE_FILE" ]; then
    ...
    sed -e "s/\${BIBLIO_HUB_HOSTNAME}/$BIBLIO_HUB_HOSTNAME/g" \
        ...
        -e "s/\${CATALOG_OIDC_CLIENT_SECRET}/$CATALOG_OIDC_CLIENT_SECRET/g" \
